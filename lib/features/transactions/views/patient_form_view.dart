...
class PatientFormView extends ConsumerWidget {
  PatientFormView({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Patientuppgifter'),
      ),
      body: Padding(
          ...
          child: Column(
            children: [
              ...
              FormBuilderTextField(
                ...
                textCapitalization: TextCapitalization.words,
                decoration: const InputDecoration(label: Text('Efternamn')),
              ),
              addHorizontalSpace(),
              ElevatedButton(
                onPressed: ref.watch(isButtonActive) ? () {
                  final patient = _formKey.currentState!.value;
                  ref.read(patientStateProvider.notifier).state = Patient.fromMap(patient);
